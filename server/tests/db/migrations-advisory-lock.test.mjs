 ...
 * schema_migrations ar putea da unique-violation / cursă.
 *
 * Testul:
...
const d = describe.skipIf(!hasTestDb())('runMigrations — advisory lock', () => {
  beforeAll(async () => {
    await migrate();
    // Marchează toate fișierele V4 ca applied → runMigrations nu re-aplică nimic,
    // ca să izolăm testul pe mecanismul de lock.
  ...
  it('014_alop rămâne marcat applied', async () => {
