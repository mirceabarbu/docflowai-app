...
// ── GET /verify/:flowId — verificare publică document după Flow ID ────────
// Fără autentificare — accesibil oricui (destinatari externi, instanțe etc.)
router.get('/verify/:flowId', async (req, res) => {
  try {
    ...
    // Date publice — fără informații private
    const signers = (data.signers || []).map(s => ({
      ...
      ancoreFieldName: s.ancoreFieldName || null,
    }));

    // Hash-uri semnătari din audit (signaturesHash per semnatar)
    const events = (data.events || []).filter(e =>
      ['FLOW_CREATED','SIGNED','SIGNED_PDF_UPLOADED','FLOW_COMPLETED','FLOW_CANCELLED','REFUSED'].includes(e.type)
    ...
    return res.json({
      ...
      signers,
      events,
      // Hash-uri documente semnate (integritate)
      signedPdfHash: data.signedPdfHash || null,
    });
  } catch(e) {
    logger.error({ err: e }, 'verify flow error');
    return res.status(500).json({ error: 'server_error' });
  }
});

// ── POST /verify/signature — verificare criptografică PDF semnat ──────────
// Acceptă un PDF uploadat și returnează rezultatul verificării complete.
router.post('/verify/signature', async (req, res) => {
  try {
    ...
    if (pdfBytes.length > 50 * 1024 * 1024)
      return res.status(413).json({ error: 'pdf_too_large', message: 'Max 50MB.' });

    const { verifyPdfSignatures, formatVerificationResult } = await import('../verify.mjs');
    const raw_result = await verifyPdfSignatures(pdfBytes);
    const result     = formatVerificationResult(raw_result);

    // Dacă avem flowId, îl corelăm cu DB-ul
    let dbData = null;
    if (flowId) {
      try {
        const data = await getFlowData(flowId);
    ...
    logger.info({ flowId, sigCount: result.signatureCount, ok: result.ok }, 'PDF verified');
    return res.json({ ...result, dbData });

  } catch(e) {
    logger.error({ err: e }, 'verify signature error');
