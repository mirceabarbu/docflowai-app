...
// ── FEAT-01 v3.3.8: Webhook config per organizație ───────────────────────────

// GET /admin/webhook — citește configurația curentă
router.get('/admin/webhook', async (req, res) => {
  if (requireDb(res)) return;
  const actor = requireAuth(req, res); if (!actor) return;
  if (!isAdminOrOrgAdmin(actor)) return res.status(403).json({ error: 'forbidden' });
  try {
    ...
    const { rows } = await pool.query(
      'SELECT id, name, webhook_url, CASE WHEN webhook_secret IS NOT NULL THEN true ELSE false END AS has_secret FROM organizations WHERE id=$1',
      [orgId]
    );
    if (!rows.length) return res.status(404).json({ error: 'org_not_found' });
    res.json({ ok: true, org: rows[0] });
  } catch(e) { logger.error({ err: e }, 'GET /admin/webhook error'); res.status(500).json({ error: 'server_error' }); }
});

// PUT /admin/webhook — setează / șterge webhook_url și webhook_secret
router.put('/admin/webhook', async (req, res) => {
  if (requireDb(res)) return;
  const actor = requireAuth(req, res); if (!actor) return;
  if (!isAdminOrOrgAdmin(actor)) return res.status(403).json({ error: 'forbidden' });
  ...
  if (webhook_url !== null && webhook_url !== undefined && webhook_url !== '') {
    if (typeof webhook_url !== 'string') return res.status(400).json({ error: 'webhook_url_must_be_string' });
    if (webhook_url.length > 2048) return res.status(400).json({ error: 'webhook_url_too_long' });
    try { const u = new URL(webhook_url); if (!['https:', 'http:'].includes(u.protocol)) throw new Error('not http'); }
    catch(e) { return res.status(400).json({ error: 'webhook_url_invalid', message: 'URL-ul trebuie să fie http:// sau https://' }); }
  }
  ...
  try {
    ...
    const newSecret = webhook_secret || null;

    await pool.query(
      'UPDATE organizations SET webhook_url=$1, webhook_secret=$2 WHERE id=$3',
      [newUrl, newSecret, orgId]
    );

    writeAuditEvent({ flowId: null, orgId, eventType: 'WEBHOOK_CONFIG_UPDATED',
      actorEmail: actor.email, payload: { webhook_url: newUrl ? '***' : null, has_secret: !!newSecret } });

    logger.info({ orgId, actor: actor.email, hasUrl: !!newUrl }, 'Webhook config actualizat');
    res.json({ ok: true, webhook_url: newUrl, has_secret: !!newSecret });
  } catch(e) { logger.error({ err: e }, 'PUT /admin/webhook error'); res.status(500).json({ error: 'server_error' }); }
});

// GET /admin/webhook/deliveries — status livrări recente (ultimele 50)
router.get('/admin/webhook/deliveries', async (req, res) => {
  ...
  try {
    ...
    if (!orgId) return res.status(400).json({ error: 'no_org' });
    const { rows } = await pool.query(
      `SELECT id, flow_id, event_type, status, attempts, last_error, next_retry, delivered_at, created_at
       FROM webhook_deliveries WHERE org_id=$1 ORDER BY created_at DESC LIMIT 50`,
      [orgId]
    );
    res.json({ ok: true, deliveries: rows });
  } catch(e) { logger.error({ err: e }, 'GET /admin/webhook/deliveries error'); res.status(500).json({ error: 'server_error' }); }
});

// POST /admin/webhook/test — trimite un payload de test la webhook_url curent
router.post('/admin/webhook/test', async (req, res) => {
  ...
  try {
    const orgId = actor.orgId;
    if (!orgId) return res.status(400).json({ error: 'no_org' });
    const { rows } = await pool.query('SELECT webhook_url, webhook_secret FROM organizations WHERE id=$1', [orgId]);
    const org = rows[0];
    if (!org?.webhook_url) return res.status(400).json({ error: 'no_webhook_url', message: 'Configurați mai întâi un webhook URL.' });

    // Payload de test — nu afectează DB
    const testPayload = {
      event: 'TEST', flowId: 'TEST_FLOW', orgId,
      docName: 'Document de test', flowType: 'tabel',
      completedAt: new Date().toISOString(),
      signersCount: 1,
      signers: [{ name: 'Test Semnatar', email: 'test@example.com', status: 'signed', signedAt: new Date().toISOString() }],
      ts: new Date().toISOString(),
    };

    const payloadStr = JSON.stringify(testPayload);
    const sig = org.webhook_secret
      ? 'sha256=' + (await import('crypto')).default.createHmac('sha256', org.webhook_secret).update(payloadStr).digest('hex')
      : null;
    ...
    try {
      const resp = await fetch(org.webhook_url, {
        ...
        headers: {
          ...
          'X-DocFlowAI-Event': 'TEST',
          ...(sig ? { 'X-DocFlowAI-Signature': sig } : {}),
        },
        body: payloadStr,
      ...
      res.json({ ok: false, error: fetchErr.name === 'AbortError' ? 'timeout_10s' : String(fetchErr.message) });
    }
  } catch(e) { logger.error({ err: e }, 'POST /admin/webhook/test error'); res.status(500).json({ error: 'server_error' }); }
});
