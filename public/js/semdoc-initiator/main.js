        ...
        sortedProviders.forEach(p => {
          ...
          label.innerHTML = `
            ...
            <span style="font-size:.84rem; color:${isSel ? meta.color : 'var(--sub)'}; font-weight:600; display:inline-flex; align-items:center; gap:5px;">
              <svg class="df-ico df-ico-sm" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#${meta.iconId}"/></svg>${esc(p.label)}
            </span>
          `;
      ...
      function showTemplatePicker(templates) {
        ...
        box.innerHTML = `
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            ...
            <button onclick="document.getElementById('tmplPickerOverlay').remove()" class="df-action-btn icon-only sm" aria-label="Închide"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-x"/></svg></button>
          </div>
      ...
      const dlToken = ""; // unused — cookie trimis automat cu credentials: include
        ...
        el.innerHTML = flows.map(f => {
          ...
          const dlBtn = pdfReady
            ? `<a href="/flows/${encodeURIComponent(f.flowId)}/signed-pdf" class="df-action-btn primary"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-download"/></svg>PDF semnat</a>
               <button onclick="downloadTrustReportInit('${f.flowId}', this)" class="df-action-btn"><svg class="df-ico df-ico-sm" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-file-text"/></svg>Raport conformitate</button>`
            : successFinal
          ...
          return `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:18px 20px;margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;gap:10px;flex-wrap:wrap;">
              ...
              <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                ...
                ${!isCancelled && mySignerEntry ? `<button onclick="signFromFluxuri('${f.flowId}')" class="df-action-btn cta sm"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-pen-tool"/></svg>Semnează</button>` : ''}
                <a href="/flow.html?flow=${f.flowId}" class="df-action-btn"><svg class="df-ico df-ico-sm" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-search"/></svg>Vezi flow</a>
                ${!isCancelled ? dlBtn : ''}
                ${pdfReady ? `<button onclick="_openEmailForFlow('${f.flowId}')" class="df-action-btn success" title="Trimite pe email extern"><svg class="df-ico df-ico-sm" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-mail"/></svg>Trimite</button>` : ''}
                ${!isCancelled && canReinitiate && (f.initEmail||'').toLowerCase() === currentUserEmail ? `<button onclick="reinitiateFlow('${f.flowId}','${(f.docName||'').replace(/'/g,"\'")}')" class="df-action-btn warning sm"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-refresh"/></svg>Reinițiază</button>` : ''}
                ...
                ${!isCancelled && isReviewRequested && (f.initEmail||'').toLowerCase() === currentUserEmail ? `<button onclick="showReviewUploadModal('${f.flowId}')" class="df-action-btn warning sm"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-upload"/></svg>Doc revizuit</button>` : ''}
                ...
                ${!isCancelled && (!isRefused || canReinitiate) && !f.allSigned && !f.completed && (f.initEmail||'').toLowerCase() === currentUserEmail ? `<button onclick="cancelFlow('${f.flowId}','${(f.docName||'').replace(/'/g,"\'")}')" class="df-action-btn sm" title="Anulează flux"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-ban"/></svg>Anulează</button>` : ''}
              </div>
            </div>
      ...
      function _renderAttachList() {
        ...
        list.innerHTML = _attachFiles.map((af, i) => `
          <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:rgba(124,92,255,.1);border-radius:6px;font-size:.8rem;">
            ...
            <button onclick="_removeAttach(${i})" class="df-action-btn icon-only sm" title="Elimină" aria-label="Elimină"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.472#ico-x"/></svg></button>
          </div>`).join('');
      }
