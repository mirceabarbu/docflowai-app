...
function buildCard(t,idx) {
  ...
  const shareBtnContent = t.shared
    ? '<svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.475#ico-shield"/></svg>Fă privat'
    : '<svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.475#ico-building"/></svg>Share';
  const actions = t.isOwner ? `
    <button class="df-action-btn sm" onclick='openEdit(${JSON.stringify(t)})'><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.475#ico-edit"/></svg>Editează</button>
  ...
  div.innerHTML=`
    ...
    <div class="tmpl-actions">
      <button class="df-action-btn cta sm" onclick="useTemplate(${t.id})"><svg class="df-ic" viewBox="0 0 24 24"><use href="/icons.svg?v=3.9.475#ico-play"/></svg>Folosește</button>
      ${actions}
    </div>`;
  return div;
}
