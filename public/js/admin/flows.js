            ...
            return `<tr style="border-bottom:1px solid rgba(255,255,255,.04);">
              ...
              <td style="padding:7px 8px;">
                ...
                ${current&&!f.completed&&f.status!=="refused" ? `
                  <button class="df-action-btn sm" onclick="resendNotif('${f.flowId}')" style="margin-right:4px;background:rgba(124,92,255,.2);border-color:rgba(124,92,255,.3);color:#b39dff;" title="Retrimite notificare">📨</button>
                  ${expired?`<button class="df-action-btn warning sm" onclick="regenerateToken('${escH(f.flowId||'')}','${escH(current.email||'')}')" style="margin-right:4px;" title="Generează token nou">🔑</button>`:''}
                ` : ''}
                <button class="df-action-btn danger sm" onclick="adminDeleteFlow('${escH(f.flowId||'')}')" title="Șterge flux definitiv (ireversibil)">🗑</button>
                <button class="df-action-btn teal sm" onclick="downloadAudit('${escH(f.flowId||'')}','txt')" title="Export audit TXT">📋</button><button class="df-action-btn teal sm" onclick="downloadAudit('${escH(f.flowId||'')}','pdf')" style="margin-left:4px;" title="Export audit PDF">📄</button>${f.completed ? `<button class="df-action-btn sm" onclick="downloadTrustReport('${escH(f.flowId||'')}', this)" style="margin-left:4px;background:rgba(124,92,255,.12);border-color:rgba(124,92,255,.3);color:#b39dff;" title="Signing Trust Report">📜</button>` : ''}
              </td>
            </tr>`;
          }).join("")}
          </tbody>
