...
function _alopStatusBadge(status, dfFlowId){
  ...
  const _ico=`<svg width="11" height="11" style="vertical-align:-1px;margin-right:4px;flex-shrink:0;"><use href="/icons.svg?v=3.9.470#${s.icon}"/></svg>`;
    ...
    tb.innerHTML=rows.map(a=>{
      ...
      return`<tr onclick="openAlop('${esc(a.id)}')" style="cursor:pointer">
        ...
        <td onclick="event.stopPropagation()">
          ...
          ${a.has_opme_lines?`<button class="df-action-btn sm" style="margin-left:4px" onclick="openOpmeLinesForAlop('${esc(a.id)}')" title="Vezi OP-uri OPME atașate"><svg class="df-ico"><use href="/icons.svg?v=3.9.470#ico-landmark"/></svg></button>`:''}
...
const _alopIcoBtn = (name) =>
  `<svg class="df-ic"><use href="/icons.svg?v=3.9.470#${name}"/></svg>`;
...
function renderAlopDetail(a,container){
  ...
  if(!isCompleted&&!isCancelled&&isAlopOwner){
    ...
    if(dfInLucru){
      ...
      actionsHtml+=`<span style="color:var(--df-text-3);font-size:.85rem"><svg class="df-ic" style="vertical-align:-3px;margin-right:4px;"><use href="/icons.svg?v=3.9.470#ico-clock"/></svg>DF pe fluxul de semnare — în așteptare</span>`;
